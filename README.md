...
免杀：
  ...
  **新增：**       
    - 两层认证系统：企业风格伪装登录页 + C2 暗色登录（防指纹识别）     
    - 监听器动态管理：Web 界面实时添加/删除 TCP / WSS 监听端口    
    - Payload 生成器升级：支持 WSS EXE/DLL、Linux ELF（x86_64）    
    - WSS 证书自动生成（auto_cert），证书缺失时自动签发       
 
  **优化：**     
    - Server 单文件打包（PyInstaller），Windows/Linux 均开箱即用   
    - 支持 config.yaml / 环境变量 / CLI 参数三层配置覆盖   
    - 登录失败锁定机制，防暴力破解   

  **修复：**   
    - 修复 WSS 长时间命令后断连问题   
    - 修复命令输出 GBK→UTF-8 编码乱码    
